...
// +----------------------------------------------------------------------
	...
	function get_invite_list($limit,$user_id,$type=0)
	{
		$user_id = intval($user_id);
		//一年前
		$after_year =  next_replay_month(to_timespan(to_date(TIME_UTC,"Y-m-d")),-12);
		
		if($type==0){
			$condition =" AND u.create_time >= ".$after_year." and dl.create_time  > ".$after_year."  AND dl.user_id > 0";
		}
		else
		{
			$condition =" AND (u.create_time < ".$after_year." OR dl.user_id is null ) ";
		}
		
		$sql_count = "select count(DISTINCT u.id) from ".DB_PREFIX."user u LEFT JOIN ".DB_PREFIX."deal_load dl ON dl.user_id=u.id and dl.is_repay=0  where u.pid = ".$user_id." ".$condition." ";
				
		$count = $GLOBALS['db']->getOne($sql_count);
		$list = array();
		if($count > 0){
			$sql = "select u.*,dl.user_id as r_user_id,dl.create_time as r_create_time FROM ".DB_PREFIX."user u LEFT JOIN ".DB_PREFIX."deal_load dl ON dl.user_id=u.id and dl.is_repay=0  where u.pid = ".$user_id." $condition group by u.id order by id DESC limit ".$limit;
		...
		if ($user_id > 0){
			...
			switch($mode){
				...
				case "over" :
					...
					break;
				case "bad" :
					$condtion = "   AND d.deal_status = 4 AND (".TIME_UTC." - last_repay_time)/24/3600 >=".trim(app_conf("YZ_IMPSE_DAY"))." and last_repay_time > 0 ";
					break;
			}
			...
			if($count >0){
				...
				foreach($list as $k=>$v){
					...
					if($v['deal_status'] == 4 || $v['deal_status'] == 5){
						$durl = "/index.php?ctl=uc_invest&act=mrefdetail&id=".$v['id']."&user_name=".$user_name."&user_pwd=".$user_pwd;					
						$list[$k]['app_url'] = str_replace("/mapi", "", SITE_DOMAIN.$durl);
					}else{
						$durl = "/index.php?ctl=deal&act=mobile&id=".$v['id'];
						$list[$k]['app_url'] = str_replace("/mapi", "", SITE_DOMAIN.$durl);
	...
	function getUcTransferList($page,$status){
		...
		if($rs_count > 0){
			$list_sql = 'SELECT dl.id as dlid,d.*,dl.money as load_money,dlt.id as dltid,dlt.status as tras_status,dlt.t_user_id,dlt.transfer_amount,dlt.create_time as tras_create_time FROM '.DB_PREFIX.'deal_load dl LEFT JOIN '.DB_PREFIX.'deal d ON d.id = dl.deal_id '.$union_sql.' WHERE 1=1 '.$condition;
			...
			foreach($list as $k => $v){
				...
				if($v['loantype'] == 0){
					...
					//剩余多少本金未回
					if($v['repay_time'] == $list[$k]['how_much_month'])
						$list[$k]['left_benjin'] = $v['load_money'];
					else
						$list[$k]['left_benjin'] = get_benjin($v['repay_time']-$list[$k]['how_much_month'],$v['repay_time'],$v['load_money'],$list[$k]['month_repay_money'],$v['rate']);
				}
				//每月付息，到期还本	
				elseif($v['loantype'] == 1){
		...
		if($transfer){
			...
			if($transfer['loantype'] == 0){
				...
				if($transfer['repay_time']==$transfer['how_much_month'])
					...
					$transfer['left_benjin'] = get_benjin($transfer['repay_time']-$transfer['how_much_month'],$transfer['repay_time'],$transfer['load_money'],$transfer['month_repay_money'],$transfer['rate']);
