...
function get_can_use_quota($uid){
	//用户的总额度
	$quota = $GLOBALS['db']->getOneCached("select quota from ".DB_PREFIX."user where id = ".$uid);
	//获取用户借款用去的额度
	$borrow_quota = $GLOBALS['db']->getOneCached("select sum(borrow_amount) from ".DB_PREFIX."deal where is_delete=0 AND publish_wait=0 AND deal_status in(0,1,2,4) AND user_id = ".$uid);
