...
class userModule extends SiteBaseModule
{
	...
	public function doregister()
	{
		...
		if($user_data['pid'] > 0){
			...
			if($refer_count == 0)
				$user_data['referral_rate'] = 10;
			elseif($refer_count > 10)
				$user_data['referral_rate'] =20;
			else
				$user_data['referral_rate'] =10 + $refer_count;
		}
		...
		if((app_conf("REGISTER_TYPE") == 0 || app_conf("REGISTER_TYPE") == 1) && (app_conf("USER_VERIFY") == 0 || app_conf("USER_VERIFY") == 2)){
			...
			//判断验证码是否正确
			if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."mobile_verify_code WHERE mobile='".strim($user_data['mobile'])."' AND verify_code='".strim($user_data['sms_code'])."'")==0){
				showErr("手机验证码出错");
			}
		...
		if((app_conf("REGISTER_TYPE") == 0 || app_conf("REGISTER_TYPE") == 2) && (app_conf("USER_VERIFY") == 1 || app_conf("USER_VERIFY") == 2)){
			...
			//判断验证码是否正确
			if($GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."email_verify_code WHERE email='".strim($user_data['email'])."' AND verify_code='".strim($user_data['emsms_code'])."'")==0){
				showErr("邮箱验证码出错");
			}
			$user_data['is_effect'] = 1;
