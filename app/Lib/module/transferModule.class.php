...
class transferModule extends SiteBaseModule
{
	public function index(){
		...
		if($cate_id > 0){
			...
			switch($field){
				case "borrow_amount":
					$field = "dlt.transfer_amount";
					break;
				case "rate":
					$field = "d.rate";
					break;
				case "repay_time":
					$field = "dlt.last_repay_time";
					break;
				case "remain_time":
					$field = "dlt.near_repay_time";
					break;
			}
			if($field && $field_sort)
				$orderby = "$field $field_sort ,d.deal_status desc , d.sort DESC,d.id DESC";
			else
		...
		elseif ($cate_id == 0){
			...
			if($field && $field_sort)
				$orderby = "$field $field_sort, dlt.create_time DESC , dlt.id DESC ";
			else
		...
		if($GLOBALS['user_info']){
			...
			if($deal['deal_status'] >=4){
				...
				foreach($load_list as $k=>$v){
					$load_list[$k]['remain_money'] = $v['money'] - $GLOBALS['db']->getOne("SELECT sum(self_money) FROM ".DB_PREFIX."deal_load_repay WHERE user_id=".$v['user_id']." AND deal_id=".$deal_id);
