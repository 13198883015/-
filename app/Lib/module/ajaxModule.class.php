...
class ajaxModule extends SiteBaseModule
{
	...
	public function unit_get_email_verifyss()
	{
		...
		$user_info =  $GLOBALS['db']->getOne("select * from ".DB_PREFIX."deal_agency where email='".$email."'");
		if(!$user_info){
			$data['status'] = 0;
			$data['info'] = "邮箱对应会员不存在";
			ajax_return($data);
	...
	public function get_email_verifys()
	{
		...
		//开始生成邮箱验证码
		$user= $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where email = '".$new_email."' ");
		if($user)
		{
	...
	public function get_unit_email_verifys()
	{
		$new_email = strim($_REQUEST['user_email']);
		//开始生成邮箱验证码
		$user= $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_agency where email = '".$new_email."' ");
		if($user)
		{
			$data['status'] = 0;
			$data['info'] = "该邮箱已被其他用户绑定";
		...
		$code = rand(111111,999999);
		$GLOBALS['db']->query("update ".DB_PREFIX."deal_agency set verify = '".$code."',verify_create_time = '".TIME_UTC."' where id = ".$user_id);
	...
	public function get_register_verify_code()
	{
		...
		if(!check_ipop_limit(get_client_ip(),"register_mobile_verify",60,0))
		{
			$data['status'] = 0;
	...
	public function unit_get_re_pwd_verify_code()
	{
		...
		if($users = $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."deal_agency WHERE mobile='".$user_mobile."'")){
			/*if($users['is_delete'] == 1 || $users['is_effect']==0){
				$data['status'] = 0;
				$data['info'] = "会员被锁定或者删除";
				ajax_return($data);
	...
	*/
		if($users = $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."deal_agency WHERE email='".$user_email."'")){
	...
	public function get_unit_verify_code()
	{
		...
		//查询是否有用户绑定
		$user= $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_agency where mobile = '".$user_mobile."' and mobilepassed = 1");
		...
		$code = rand(111111,999999);
		$GLOBALS['db']->query("update ".DB_PREFIX."deal_agency set bind_verify = '".$code."',verify_create_time = '".TIME_UTC."' where id = ".$user_id);
		send_verify_sms($user_mobile,$code,$GLOBALS['manageagency_info']);
		$data['status'] = 1;
		$data['info'] = $GLOBALS['lang']['MOBILE_VERIFY_SEND_OK'];
...
//手机绑定
	...
	public function check_unit_verify_code(){
		...
		$user = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_agency where id = ".intval($GLOBALS['manageagency_info']['id']));
		$user_mobile = addslashes(htmlspecialchars(trim($_REQUEST['mobile'])));	
		if(app_conf("SMS_ON")==1)
		{	
			if($user['bind_verify'] == '' || $user['bind_verify']!=$verify)
			...
			{
				$GLOBALS['db']->query("update ".DB_PREFIX."deal_agency set mobile='$user_mobile',mobilepassed=1, bind_verify = '', verify_create_time = 0 where id = ".intval($GLOBALS['manageagency_info']['id']));
				showSuccess($GLOBALS['lang']['MOBILE_BIND_SUCCESS'],$ajax);
			}
		}
		else{
	...
	public function get_paypwd_verify_code()
	{
		...
		//开始生成手机验证
		//$code = $GLOBALS['db']->getOne("select bind_verify from ".DB_PREFIX."user where id = ".$user_id);
		//if (intval($code) == 0){
		...
		//手机验证，使用立即发送方式;
		$result = send_verify_sms($user_mobile,$code,$GLOBALS['user_info']);//		
	...
	public function check_paypwd_verify_code(){
		...
		$user = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."user where id = ".intval($GLOBALS['user_info']['id']));
		$paypassword = addslashes(htmlspecialchars(trim($_REQUEST['paypassword'])));	
		if(app_conf("SMS_ON")==1)
		{	
			if($user['bind_verify'] == '' || $user['bind_verify']!=$verify)
			{
	...
	public function check_event()
	{
		if($GLOBALS['user_info'])
		{
			...
			if($event)
			{
				if($event['submit_begin_time']>TIME_UTC)
				{
					$result['status'] = 2;
					$result['info'] = $GLOBALS['lang']['EVENT_NOT_START'];
				}
				elseif($event['submit_end_time']<TIME_UTC&&$event['submit_end_time']!=0)
				{
					$result['status'] = 2;
			...
			if($event)
			{
				...
				else
				{
					...
					{						
						...
						$submit_data['create_time'] = TIME_UTC;
...
public function drop_pm()
		...
		else
		{
			...
			else
			{
				...
				if($point>0||$score>0||$money>0)
				{
					...
					modify_account($data,$user_id,"您在".to_date(TIME_UTC)."签到成功");
	...
	function check_user_info(){
		...
		if ($user_type == 0){
			...
			$data = $GLOBALS['db']->getRow("select * from ".DB_PREFIX."deal_agency where id = ".$user_id);
		}
