...
class uc_inviteModule extends SiteBaseModule
{
	public function index()
	{
		...
		$referral_user = $GLOBALS['db']->getOne("select count(*) from ".DB_PREFIX."user where pid = ".$GLOBALS['user_info']['id']." and is_effect=1 and is_delete=0");
	...
	function reward(){
		...
		$result['count'] = $GLOBALS['db']->getOne("SELECT count(*) FROM ".DB_PREFIX."referrals WHERE user_id=".$GLOBALS['user_info']['id']." ORDER BY id DESC");
		if($result['count'] > 0){
			$result['list'] = $GLOBALS['db']->getAll("SELECT r.*,u.user_name as re_user_name,u.referral_rate FROM ".DB_PREFIX."referrals r LEFT JOIN ".DB_PREFIX."user u ON u.id = r.rel_user_id WHERE r.user_id=".$GLOBALS['user_info']['id']." ORDER BY id DESC");
		}
