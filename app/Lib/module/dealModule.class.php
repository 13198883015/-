...
class dealModule extends SiteBaseModule
{
	public function index(){
		...
		$deal_info = $GLOBALS['db']->getRow("SELECT * FROM ".DB_PREFIX."deal where id = ".$deal['id']);
		$deal_infos = $deal_info['view_info'];
		
		if($deal_infos!=""){
			$view_info_list = unserialize($deal_infos);
	...
	function preview(){
		...
		$deal['borrow_amount_format'] = format_price(strim($_REQUEST['borrowamount']));
		...
		//本息还款金额
		$deal['month_repay_money'] = format_price(pl_it_formula($deal['borrow_amount'],strim($_REQUEST['apr'])/12/100,$deal['repay_time']));
