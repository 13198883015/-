...
function format_deal_item(&$deal,$user_name="",$user_pwd=""){
	...
	if($deal['deal_status']>=4){
		...
		//总的必须还多少本息
		if($deal['loantype'] == 0)
			$deal['remain_repay_money'] = floatval($deal['month_repay_money']) * $true_repay_time;
		elseif($deal['loantype'] == 1)//每月还息，到期还本
			$deal['remain_repay_money'] = floatval($deal['borrow_amount']) + floatval($deal['month_repay_money']) * $true_repay_time;
		elseif($deal['loantype'] == 2)
			$deal['remain_repay_money'] = floatval($deal['borrow_amount']) + floatval($deal['month_repay_money']);
		...
		if($deal['deal_status']==4){
			...
			if($deal['last_repay_time'] > 0){
				...
				if($deal['repay_time_type'] == 0){
					$deal["next_repay_time"] = $deal['repay_start_time'] + $deal['repay_time']*24*3600;
				}
				else{
...
function get_deal_load_list($deal){
	...
	foreach($deal_repay_list as $k=>$v){
		...
		if($v['has_repay'] == 1){
			...
			//真实还多少
			$loan_list[$i]['month_has_repay_money_all'] = $loan_list[$i]['month_has_repay_money'] + $loan_list[$i]['month_manage_money']+$loan_list[$i]['impose_money'];
		...
		elseif($v['has_repay'] == 0){
			...
			elseif(to_date(TIME_UTC,"Y-m-d") == to_date($v['repay_time'],"Y-m-d")){
...
function get_deal_user_load_list($deal_info, $user_id = 0 ,$lkey = -1 , $ukey = -1,$true_time=0,$get_type = 0, $r_type = 0, $limit = ""){
	...
	foreach($load_users as $k=>$v){
			...
			if($v['has_repay'] == 0){
				...
				elseif(to_date($true_time,"Y-m-d") == to_date($v['repay_time'],"Y-m-d")){
					$item['status'] = 2;
				}
				else{
					$item['status'] = 1;
...
function get_transfer($union_sql,$condition){
	...
	$sql = 'SELECT dlt.id,dlt.transfer_amount,dlt.near_repay_time,dlt.user_id,d.loantype,d.next_repay_time,d.last_repay_time,d.rate,d.repay_start_time,d.repay_time,d.name as deal_name,dlt.load_money,dlt.id as dltid,dlt.status as tras_status,dlt.t_user_id,dlt.transfer_amount,dlt.create_time as tras_create_time,dlt.transfer_time,dlt.load_id FROM '.DB_PREFIX.'deal_load dl LEFT JOIN '.DB_PREFIX.'deal d ON d.id = dl.deal_id '.$union_sql.' WHERE 1=1 '.$condition;
	...
	if($transfer){
		...
		//每月付息，到期还本	
		elseif($transfer['loantype'] == 1){
			$transfer['month_repay_money'] = av_it_formula($transfer['load_money'],$transfer['rate']/12/100) ;
			//剩余多少钱未回
			$transfer['all_must_repay_money'] = $transfer['month_repay_money'] * $transfer['how_much_month'] + $transfer['load_money'];
			//剩余多少本金未回
			$transfer['left_benjin'] = $transfer['load_money'];
		}
		//到期还本息
		elseif($transfer['loantype'] == 2){
			$transfer['month_repay_money'] = 0 ;
			//剩余多少钱未回
			$transfer['all_must_repay_money'] = ($transfer['load_money'] * $transfer['rate']/12/100) * $transfer['repay_time'] + $transfer['load_money'];
			//剩余多少本金未回
			$transfer['left_benjin'] = $transfer['load_money'];
		}	
...
function getUcRepayBorrowMoney($id,$ids){
	...
	if($deal['ips_bill_no']!=""){
		$root["status"] = 2;
		$root["jump"] = APP_ROOT.'/index.php?ctl=collocation&act=RepaymentNewTrade&deal_id='.$deal['id'].'&l_key='.$ids;
		return $root;
	}
	if($deal['user_id']!=$GLOBALS['user_info']['id']){
		$root["show_err"] = "不属于你的借款！";
	...
	//当前用户余额
	$user_total_money = $GLOBALS['user_info']['money'];
	...
	foreach($ids as $lkey){
		...
		foreach($user_loan_list as $lllk=>$lllv){
			foreach($lllv as $kk=>$vv){
				if($vv['has_repay']==0 ){//借入者已还款，但是没打款到借出用户中心
					...
					$user_load_data['manage_money'] = $vv['manage_money'];
					$user_load_data['impose_money'] = $vv['impose_money'];
					$user_load_data['repay_manage_impose_money'] = $vv['repay_manage_impose_money'];
					
					
					$need_repay_money = 0;
					$need_repay_money += (float)$vv['month_repay_money']  + (float)$vv['impose_money'] + (float)$vv['repay_manage_money'] + (float)$vv['repay_manage_impose_money'];
					//=============余额足够才进行还款=================
					if((float)$need_repay_money <= (float)$user_total_money){
						$last_repay_key = $lkey;
						$repay_user_count +=1;
						$repay_money +=$vv['month_repay_money'];
						$repay_impose_money += $vv['impose_money'];
						$repay_manage_money += $vv['repay_manage_money'];
						$repay_manage_impose_money += $vv['repay_manage_impose_money'];
						...
						if($GLOBALS['db']->affected_rows() > 0){
							...
							if($unext_loan){
								...
								$all_repay_money= number_format($GLOBALS['db']->getOne("SELECT (sum(repay_money)-sum(self_money) + sum(impose_money)) as shouyi FROM ".DB_PREFIX."deal_load_repay WHERE deal_id=".$deal['id']." AND user_id=".$vv['user_id']),2);
								$all_impose_money = number_format($GLOBALS['db']->getOne("SELECT sum(impose_money) FROM ".DB_PREFIX."deal_load_repay WHERE deal_id=".$deal['id']." AND user_id=".$vv['user_id']),2);
		...
		if($repay_user_count > 0){
			...
			//更新用户账户资金记录
			modify_account(array("money"=>-$repay_money),$GLOBALS['user_info']['id'],"标:".$deal['id'].",期:".($lkey+1).",偿还本息$ext_str");
			modify_account(array("money"=>-$repay_impose_money),$GLOBALS['user_info']['id'],"标:".$deal['id'].",期:".($lkey+1).",逾期罚息$ext_str");
			modify_account(array("money"=>-$repay_manage_money),$GLOBALS['user_info']['id'],"标:".$deal['id'].",期:".($lkey+1).",借款管理费$ext_str");
			modify_account(array("money"=>-$repay_manage_impose_money),$GLOBALS['user_info']['id'],"标:".$deal['id'].",期:".($lkey+1).",逾期管理费$ext_str");
		...
		//如果还款完毕
		$sms_ext_str = "成功";
		$left_user_count = $GLOBALS['db']->getOne("SELECT count(*)  FROM ".DB_PREFIX."deal_load_repay WHERE deal_id = ".$deal['id']." and l_key=".$lkey." AND has_repay = 0 ");
		if($left_user_count == 0){
			$content .="本期还款完毕。";
			$impose_rs = $GLOBALS['db']->getRow("SELECT sum(impose_money) as total_impose_money,sum(repay_manage_impose_money) as total_repay_manage_impose_money  FROM ".DB_PREFIX."deal_load_repay WHERE deal_id = ".$deal['id']." and l_key=".$lkey." AND has_repay = 1");
			//判断是否逾期
			$ext_update = ",impose_money='".$impose_rs['total_impose_money']."',mange_impose_money='".$impose_rs['total_repay_manage_impose_money']."'";
			if($vv['impose_day'] > 0){
				if($vv['impose_day'] < app_conf('YZ_IMPSE_DAY')){
					modify_account(array("point"=>trim(app_conf('IMPOSE_POINT'))),$GLOBALS['user_info']['id'],"标:".$deal['id'].",期:".($lkey+1).",逾期还款");
					$ext_update = ",status=2";
				}
				else{
					modify_account(array("point"=>trim(app_conf('YZ_IMPOSE_POINT'))),$GLOBALS['user_info']['id'],"标:".$deal['id'].",期:".($lkey+1).",严重逾期");
					$ext_update = ",status=3";
				}
			}
			
			$GLOBALS['db']->query("UPDATE ".DB_PREFIX."deal_repay SET has_repay = 1 , true_repay_time= ".TIME_UTC." $ext_update WHERE deal_id = ".$deal['id']." and l_key=".$lkey);
	...
	if($GLOBALS['db']->getOne("SELECT count(*)  FROM ".DB_PREFIX."deal_repay WHERE deal_id=".$deal['id']." and l_key=".$last_repay_key." AND has_repay <> 1)") == 0){
		...
		//全部还完
			...
			//判断获取的信用是否超过限制
			if($GLOBALS['db']->getOne("SELECT sum(point) FROM ".DB_PREFIX."user_log WHERE log_info='还清借款' AND user_id=".$GLOBALS['user_info']['id']) < (int)trim(app_conf('REPAY_SUCCESS_LIMIT'))){
				//获取上一次还款时间
...
function getUCInrepayRepayBorrowMoney($id){
	...
	if($deal['loantype']==0){//等额本息的时候才通过公式计算剩余多少本金
		...
		$impose_money = ($benjin - $deal['month_repay_money'] + $benjin*$rate) * (float)trim(app_conf('COMPENSATE_FEE'))/100;
		$total_repay_money = $benjin + $benjin*$rate;
	}
	elseif($deal['loantype']==1){//每月付息，到期还本
		$impose_money = $benjin * (float)trim(app_conf('COMPENSATE_FEE'))/100;
		$total_repay_money = $benjin + $deal['month_repay_money'];
	}
	elseif($deal['loantype']==2){//到期还本息
		$impose_money += $benjin * (float)trim(app_conf('COMPENSATE_FEE'))/100;
				...
				//当前提前还款的第一个月
					...
					if($vv['repay_day'] == $k_repay_time){
						if($deal['loantype']==0){//等额本息的时候才通过公式计算剩余多少本金
							...
							$user_load_data['impose_money'] = ($user_load_data['self_money'] - $vv['month_repay_money'] + $user_load_data['self_money']*$v['rate']) * (float)trim(app_conf('COMPENSATE_FEE'))/100;
						}
						elseif($deal['loantype']==1){//每月还息，到期还本
