<?php
	...
	function Transfer($pTransferType, $deal_id, $ref_data, $MerCode,$cert_md5,$ws_url){
		...
		$pWebUrl= SITE_DOMAIN.APP_ROOT."/index.php?ctl=collocation&act=response&class_name=Ips&class_act=Transfer";//web方式返回
		$pS2SUrl= SITE_DOMAIN.APP_ROOT."/index.php?ctl=collocation&act=notify&class_name=Ips&class_act=Transfer";//s2s方式返回		
		...
		if ($pTransferType == 5){
			...
			if ($id > 0){
				...
				$result['id'] = $id;
					...
					$detail['pTAcctType'] = 1;//转入方账户类型  否  0#机构；1#个人,
					$pTIpsAcctNo = $GLOBALS['db']->getOne("select ips_acct_no from ".DB_PREFIX."deal_agency where id = ".$deal['agency_id']);
					$detail['pTIpsAcctNo'] = $pTIpsAcctNo;//转入方 IPS 托管 账户号  否  账户类型为1时，IPS个人托管账户号  账户类型为0时，由 IPS颁发的商户号  转账类型，1：投资，此为转入方（借款人）；  转账类型，2：代偿，此为转入方（投资人）；  转账类型，3：代偿还款，此为转入方（担保方）；  转账类型，4：债权转让，此为转入方（出让方）；  转账类型，5：结算担保收益，此为转入方（担保方）；
					$detail['pTTrdFee'] = 0;// '转入方明细手续 费  否  金额单位：元，不能为负，允许为0，保留2位小数；  格式：12.00  转账类型，1：投资，此为转入方（借款人）手续费；  转账类型，2：代偿，此为转入方（投资人）手续费；  转账类型，3：代偿还款，此为转入方（担保方）手续费；  转账类型，4：债权转让，此为转入方（出让方）手续费；  转账类型，5：结算担保收益，此为转入方（担保方）手 续费； ',
		...
		if ($data['pErrCode'] == 'MG00000F'){
			...
			if ($GLOBALS['db']->affected_rows()){	
				...
				if ($pTransferType == 1){					
					...
					$sql = "update ".DB_PREFIX."deal_load_repay set t_user_id = '".$transfer['t_user_id']."' where has_repay = 0 and load_id =".intval($transfer['load_id']) + " and user_id =".intval($transfer['user_id']) + " and deal_id = ".$deal_id;
					//echo $sql;
					$GLOBALS['db']->query($sql);
