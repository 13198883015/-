<?php
	...
	function CreateNewAcct($user_id,$user_type,$MerCode,$cert_md5,$post_url){
		...
		$pWebUrl= SITE_DOMAIN.APP_ROOT."/index.php?ctl=collocation&act=response&class_name=Ips&class_act=CreateNewAcct";//web方式返回
		$pS2SUrl= SITE_DOMAIN.APP_ROOT."/index.php?ctl=collocation&act=notify&class_name=Ips&class_act=CreateNewAcct";//s2s方式返回		
	...
	function CreateNewAcctCallBack($pMerCode,$pErrCode,$pErrMsg,$str3Req){
		...
		if ($GLOBALS['db']->affected_rows()){		
			...
			if ($pErrCode == 'MG00000F'){
				$user_type = intval($GLOBALS['db']->getOne("select user_type from ".DB_PREFIX."ips_create_new_acct where ".$where));
				...
				if ($user_type == 0){
					...
					$GLOBALS['db']->query("update ".DB_PREFIX."deal_agency set ips_acct_no = '".$data['pIpsAcctNo']."' where id = ".$user_id);
				}	
				return 	$user_type;		
			}
		}
