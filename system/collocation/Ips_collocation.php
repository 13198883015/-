		...
		if($localSign==$pSign){
			...
			if ($class_act == 'CreateNewAcct'){
				...
				if ($pErrCode == 'MG00000F'){
					if($user_type == 0)
						showSuccess($pErrMsg,0,url("index","uc_center"));	
					else
						showSuccess($pErrMsg,0,SITE_DOMAIN.APP_ROOT);				
				}else{
					showErr($pErrMsg,0,SITE_DOMAIN.APP_ROOT,30);
				}
				...
				if($ipsdata)
					showSuccess($pErrMsg,0,url("index","deal",array("id"=>$ipsdata['deal_id'])));	
				else
					showSuccess($pErrMsg,0,SITE_DOMAIN.APP_ROOT);				
				...
				if($ipsdata)
					showSuccess($pErrMsg,0,url("index","uc_deal#quick_refund",array("id"=>$ipsdata['deal_id'])));
				else
					showSuccess($pErrMsg,0,SITE_DOMAIN.APP_ROOT);
				...
				if(intval($str3Req["pTransferType"])==4)
					showSuccess($pErrMsg,0,url("index","transfer#detail",array("id"=>$result['id'])));
				else
					showSuccess($pErrMsg,0,SITE_DOMAIN.APP_ROOT);
			}
